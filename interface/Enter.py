...
class EnterInterface(MatrixInterface):
    ...
    def create_matrix(self, text, enter_type):
        ...
        try:
            ...
            raise ErrorValue("Podano niepoprawne wartosci")
        except Exception:
            raise ErrorValue("Wystapil nieznany blad")

        if len(matrix) < 1:
            raise ErrorValue("Podane macierz ma niepoprawna wielkosc")

        if enter_type == EnterStates.FIRST:
            self._first_row = len(lines)
            if self._first_row < 1:
                raise ErrorValue("Podana macierz ma niepoprawna wielkosc")
        ...
        for row in matrix:
            if enter_type == EnterStates.FIRST and len(row) != self._first_column:
                raise ErrorValue("Kolumny macierzy maja rozna wielkosc")
